function SecPaperBCR(cover_image, watermark_logo, cover_image_filename)

    % Initialize a structure to store BCR values
    results = struct('Attack', {}, 'WatermarkSize', [], 'BCR', []);
    ...
    for attackIndex = 1:length(attacks)
        ...
        for watermark_size = watermark_sizes
            ...
            % Calculate and store BCR (complement of BER)
            ...
            % Store BCR results in the structure
    ...
    % Convert the BCR structure to a table after collecting all data
    results_table = struct2table(results);

    % Create a unique Excel file for each cover image
    excel_filename = 'BCR_Results.xlsx';
