...
function [watermarked_image, extracted_watermark] = Him_dwt_hd_svd(cover_image,watermark_logo,alpha,attack,param)
...
no_sig_val = size(HSw, 1);
vect = alpha*(1:no_sig_val)/no_sig_val;
newVect = vect(end:-1:1);
new_alpha = diag(newVect); 
HSw_hat = HSw + new_alpha.*Sw;
